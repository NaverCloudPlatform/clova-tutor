...
export function MultipleChoiceItem({
  ...
  const isSelected = selectedValues.includes(option.no);
  const wasAnswered = lastAnswers.includes(option.no);
  const choiceItemStatus = !wasAnswered
  ...
  const uniqueId = `multiple-option-${chatId}-${problemId}-${option.no}`;

  return (
    <div className="flex flex-col gap-y-[0.5em] cursor-pointer">
      <Checkbox id={uniqueId} checked={isSelected} onCheckedChange={onCheckedChange} className="sr-only" />
