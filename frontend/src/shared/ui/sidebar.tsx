...
function SidebarProvider({
  ...
  // Reset openMobile when switching from mobile to desktop
  React.useEffect(() => {
    if (!isMobile) {
      setOpenMobile(false);
    }
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: 나중에 바꾸자..
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: 라이브러리 기본 생성 코드라서 무시
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'text-sidebar-foreground/75 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
